> **⚠️ M0 AUDIT — FINAL STATE (2026-04-08, after primary-source reading of 4 papers + targeted searches; see M0 Fourth Addendum):**
>
> **PRIMARY-SOURCE READING COMPLETED (Fourth Addendum):** Four papers were read in full/substantially by the owner providing PDFs: Machta 2013 Science, Transtrum 2015 JCP perspective, Mattingly 2018 PNAS (bonus — introduces d_eff terminology with a DIFFERENT formula that must be cited), Brown-Bossomaier-Barnett 2022 Sci. Rep. (Potts q=2,5,7,10 via transfer entropy — prior art for the broader research program), Quinn 2023 Rep. Prog. Phys. Summary: **no paper covers CCA's specific construction**, but **Brown et al. 2022 is the prior art for the broader research program** of information-theoretic discrimination of first-order Potts transitions (using global transfer entropy, not FIM). They study Potts q=2, 5, 7, 10 and find curve-shape differences between continuous and first-order that are qualitatively similar to CCA-1c's claims. They also provide a **simple physical mechanism** (cluster interfacial length, Eqs. 5-6 in their paper) that explains their observable — **CCA currently has no comparable physical mechanism for d_eff/η**. This is the single highest-leverage scientific question the audit has surfaced: *Can CCA's d_eff/η be explained by cluster interfacial geometry, or is it measuring something structurally different?*
>
> **MATTINGLY 2018 IS A MANDATORY CITATION:** The Sethna-lineage paper Mattingly-Transtrum-Abbott-Machta 2018 PNAS defines a scalar observable called **d_eff** via a DIFFERENT formula: d_eff = Σ_r r · Ω_r where Ω_r is the weight of the maximally informative discrete prior on edges of dimension r. This is NOT the same as CCA's d_eff = (Σλ)²/Σλ² (participation ratio of FIM eigenvalue spectrum). The two formulas measure "effective dimensionality" differently. **CCA must cite Mattingly 2018 and explicitly distinguish its d_eff from Mattingly's.** This is basic scholarly hygiene, not a novelty challenge.
>
> **⚠️ FIFTH ADDENDUM (2026-04-08, deep-research pass):** Three findings substantially update the CCA prior-art map:
>
> **(A) The FIM in CCA IS the connected correlation matrix χ_ij = ⟨s_i s_j⟩ - ⟨s_i⟩⟨s_j⟩ — a textbook stat-mech object.** This is true by definition for the per-site-field parameterization. Three additional prior-art papers in this lineage have been identified:
>   - **Vinayak, Prosen, Buča, Seligman 2014, EPL 108, 20006** (arXiv:1403.7218) — proves correlation-matrix eigenvalue density at 2D Ising criticality is power-law derivable from η_critical (=1/4 for 2D Ising)
>   - **Borgs & Chayes 1996, J. Stat. Phys. 82, 1235** (arXiv:adap-org/9411001) — rigorously connects Potts connected covariance matrix eigenvalues to FK cluster representations
>   - **Saberi, Saber, Moessner 2024, PRB 110, L180102** (arXiv:2503.03472) — interaction-correlated RMT on 2D Ising using top eigenvalue as order parameter; **HIGHEST-PRIORITY SINGLE PAPER for the owner to read**, more direct prior art in CCA's mathematical framework than anything previously identified
>
> **(B) Tentative mechanism answer (Brown 2022 question):** At continuous transitions in 2D, both CCA's spectrum observables and Brown's interfacial length are governed by the same critical exponents (η_critical and cluster fractal dim d_f). They likely track each other up to scaling factors — neither is novel content; both reduce to standard critical exponents. **At first-order transitions (Potts q≥5), they decouple** because the FIM is a 2-point observable while interfacial length is a multi-point geometric observable, and scale invariance fails. **The first-order regime is CCA's candidate territory for genuine narrow novelty.** Concrete experimental check: at T=T_c for q=2, derive CCA's d_eff and η_CCA analytically from η_critical=1/4 and compare to Phase B/C numerics; if they agree, no novel content at continuous transitions.
>
> **(C) TERMINOLOGY COLLISION — CCA's η conflicts with standard stat-mech η:** Standard η is the anomalous dimension of the two-point function (C(r) ~ r^(-(d-2+η)), η=1/4 for 2D Ising). CCA's η is an isotropy indicator on the FIM eigenvalue spectrum. Different objects, same symbol, mathematically related at criticality. **MUST RENAME CCA's η** before any external writeup. Suggested: ι_CCA, η_spec, or a non-Greek symbol entirely.
>
> **(D) P17/P23 citations VERIFIED with corrections:** Amendola, Rodrigues, Kumar, Quartin 2024 MNRAS 528, 2377 (arXiv:2307.02474) confirmed real, but the wiki's "5σ rejection of k=3" was a FORECAST not current data; current GWTC-3 result is 2σ upper limit k<2.1, ~3σ tension with k=3. Cadoni et al. 2023 JCAP 11, 007 (arXiv:2306.11588) confirmed real (7-author paper), uses "universal" not "generic," and crucially their own KS analysis on the Farrah elliptical-galaxy sample prefers k=3 over k=1. Wiki wording must be tightened.
>
